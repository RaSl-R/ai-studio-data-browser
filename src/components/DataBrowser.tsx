...
import {
  ...
  Filter, RefreshCw, Save, Download, Upload, AlertTriangle
} from 'lucide-react';
...
export const DataBrowser: React.FC<DataBrowserProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between bg-white p-4 rounded-lg shadow-sm border border-slate-200">
        ...
        <div className="flex gap-2">
          <button onClick={() => setShowFilter(!showFilter)} 
            className={`p-2 rounded hover:bg-slate-100 ${showFilter ? 'bg-blue-50 text-blue-600' : 'text-slate-600'}`} 
            title="Filter">
      ...
      {message && (
        <div className={`p-3 rounded-md flex items-center gap-2 text-sm border ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
          ...
          <button onClick={handleDownload} 
            className="w-full bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm py-2 px-3 rounded flex items-center justify-center gap-2 transition-colors">
            Download CSV
          </button>
        </div>
          ...
          <div className="bg-white p-4 rounded-lg shadow-sm border border-slate-200">
            ...
            {!importMode ? (
              <button onClick={() => setImportMode(true)} 
                className="w-full bg-blue-50 hover:bg-blue-100 text-blue-700 text-sm py-2 px-3 rounded flex items-center justify-center gap-2 transition-colors">
                Replace Table Data
              </button>
            ) : (
              <div className="space-y-2">
                ...
                <label className="block w-full text-sm text-slate-500 cursor-pointer">
                  <input type="file" accept=".csv" onChange={handleImport} 
                    className="block w-full text-slate-500 file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
                </label>
                <button onClick={() => setImportMode(false)} 
